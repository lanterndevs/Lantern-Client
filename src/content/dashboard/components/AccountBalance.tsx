      ...
      <Card>
        ...
        <List disablePadding>
          ...
          <ListItem sx={{ py: 2 }}>
            ...
            <ListItemText
              primary={<Text color="black"> $ { checked ? totalBalance.toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : '********' } </Text>}
          ...
          <ListItem sx={{ py: 2 }}>
            <ListItemText
              primary={<Text color="black"> Net change of {checked ? totalChange > 0 ? <Text color="success"> { '+3.32%' } </Text>: <Text color="error"> { '-2.32%' } </Text>: '****' } this month. </Text>}
              primaryTypographyProps={{
                variant: 'body1',
                fontWeight: 'bold',
                color: 'textPrimary',
